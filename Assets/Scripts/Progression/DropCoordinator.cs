...
// 처치 하나당 드롭 타입 하나만 나가도록 세 드롭 매니저(StatDropManager/EquipmentDropManager/
// StoneDropManager)를 조율한다. 각 매니저는 더 이상 스스로 OnMonsterDied를 듣지 않고, 여기서
// 뽑힌 타입의 RollAndSpawn만 호출당한다.
//
// 드롭 여부 자체는 dropChance 하나로만 결정된다(예전 statDropBaseChance와 같은 50%). 일단
// 드롭이 뜨기로 정해지면, 그중 어느 타입이냐를 나눈다: 활성 타입 목록은 현재 메인 스테이지에
// 따라 늘어난다 - 1스테는 포션만, 2스테부터 장비도, 3스테부터 스톤도 함께 뜬다. 그 안에서
// 어느 게 뜰지는 "그 타입이 몇 번째로 해금된 스테이지의 것이냐"를 그대로 가중치로 쓴 랜덤
// 롤이 정한다(포션=1, 장비=2, 스톤=3) - 그래서 항상 가장 최근에 해금된 타입이 그 안에서
// 가장 잘 뜬다.
public class DropCoordinator : MonoBehaviour
{
    ...
    private void HandleMonsterDied(Monster monster)
    {
        ...
        float stoneWeight = (currentMainStage >= StoneDropManager.UnlockStage && stoneDropManager != null) ? 3f : 0f;

        float total = statWeight + equipWeight + stoneWeight;
        if (total <= 0f) return;
        ...
        else if (roll < statWeight + equipWeight)
        {
            equipmentDropManager.RollAndSpawn(monster);
        }
        else
        {
            stoneDropManager.RollAndSpawn(monster);
        }
    }
}
