...
// StatPotionPickup의 장비 버전. 세 박자(던지기, 안착, 달려가서 줍기)와 등급 아우라/반짝임은
// 동일하고 비주얼만 다르다: 스탯별 유리병 대신 등급 색이 입혀진 Sword/Shield 메시 하나.
// 장착/숙련 효과는 드롭 시점이 아니라 접촉 시 EquipmentDropManager.CompleteDrop에서 적용되므로,
// 드롭됐지만 끝내 닿지 못한 아이템(플레이어가 먼저 죽는 등)은 절대 효과를 지급하지 않는다.
public class EquipmentDropPickup : MonoBehaviour
{
    ...
    // 이유는 StatPotionPickup의 동일 필드 참고 - HandlePlayerDied가 던지는 도중
    // StopAllCoroutines를 호출해서 PopIdleHold에 도달하기 전에 멈춰버릴 수 있다.
    ...
    // StatPotionPickup.HandlePlayerDied와 동일한 수정 - 픽업 코루틴이 StageManager의 사망
    // 시퀀스와 무관하게 계속 돌아 죽은 플레이어에게 다가가거나 효과를 지급하던 버그.
    ...
    // StatPotionPickup과 정확히 같은 별빛/방사형 광채 텍스처지만, 두 클래스가 계층을 공유하지
    // 않아 여기서 독립적으로 생성한다(클래스별 private static 캐시) - 어차피 64x64짜리
    // 저렴한 텍스처라 공용 애셋으로 빼낼 가치가 없다.
