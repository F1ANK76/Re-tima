...
// 스테이지 안내 카드의 밝은 파스텔톤 새벽 하늘을 코드로 만든다. 색조는 씬 하늘(상단 #334A93,
// 지평선 근처 #785F98 샘플링)을 따르되 훨씬 밝게 끌어올렸다 - 게임 자체가 밝고 경쾌해서 실제 색을
// 그대로 맞추면 오히려 우울해 보인다. 그림으로 제작하지 않고 절차적으로 그리는 이유는, 마찬가지로
// 코드로 구성되는 이 UI의 나머지와 동기화를 유지하기 위해서다.
public static class TitleCardBackdrop
{
    ...
    static void PaintSky(Color[] pixels, System.Random rng)
    {
        for (int y = 0; y < Height; y++)
        {
            ...
            // 페리윙클색 쪽으로 치우치게 해서 핑크가 프레임 가운데까지 번지지 않고 지평선 띠로만
            // 남게 한다 - 가운데는 흰색 텍스트가 잘 보여야 하는 영역이다.
