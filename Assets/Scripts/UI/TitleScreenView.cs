...
// 주인공은 이미 숲을 달리고 있으니, 메뉴가 게임의 그림이 아니라 게임 그 자체다. 손 배선이나
// 팔레트 어긋남 없이 자기 껍데기를 온전히 갖도록 코드로 빌드했다.
public class TitleScreenView : MonoBehaviour
{
    ...
    private void Build()
    {
        // 해상도와 무관하게 RectTransform이 화면 전체를 정확히 꽉 채우도록.
        var rect = GetComponent<RectTransform>();
        Stretch(rect);

        // 실제 씬 위에 얹는 스크림(scrim)일 뿐이다 - 맨 위와 맨 아래만 어둡고 가운데는
        // 투명해서, 밑에 깔린 게임플레이를 가리지 않으면서 타이틀과 버튼이 잘 읽힌다.
    ...
    // 실제 씬의 나무 라인 위쪽에 고정된 배경: 왼쪽은 낮, 오른쪽은 밤 하늘로 페이드해서
    // 아직 아무것도 고르기 전에 두 스테이지(1 = 낮, 2 = 밤)를 미리 보여준다. 실제 스카이박스는
    // 씬 전체에 단일 슬라이더로 블렌드되므로(StageMoodController 참고) "절반은 낮"이 불가능해,
    // 나무에 닿을 즈음 완전히 투명해지는 그려진 띠로 흉내내고 아래 지면/캐릭터는 안 건드린다.
    private void BuildDayNightSkySplit()
    {
        ...
        // 불투명한 부분(아래 vanishFrac 참고)이 페이드 전까지 나무 라인 위 하늘 전체(이
        // 캔버스의 1920x1080 기준 해상도에서 ~330px)를 덮을 만큼 크게 잡았다 - 화면 맨 위에
        // 그어진 띠가 아니라 하늘 자체가 나뉜 것처럼 읽혀야 한다.
        rt.sizeDelta = new Vector2(0f, 460f);
    }

    // StageMoodController가 런타임에 크로스페이드하는 것과 동일한 팔레트(그 태양/안개 색) -
    // 이 화면만을 위해 지어낸 세 번째 색조가 아니라 게임이 보여주는 두 분위기 그대로 읽힌다.
    ...
    private static Sprite CreateDayNightSkySplitSprite()
    {
        ...
        // 가운데의 이음매. 넓은 그라데이션은 양쪽 다 뚜렷한 하늘로 안 보이게 만들어서 일부러
        // 좁게 잡되, 단일 열이 아니라 몇 픽셀에 걸쳐 부드럽게 처리해 화면이 늘어나 표시될 때
        // 삐죽삐죽한 선으로 앨리어싱되지 않게 한다.
        const float seamHalfWidth = 0.025f;
        // 이 높이 비율 아래로는 완전히 투명해진다. 띠 대부분에 걸쳐 페이드하는 대신 작게
        // 잡아서, 두 하늘이 트인 영역 거의 전체에서 선명하게 갈라진 채 유지되고 나무 라인에
        // 닿는 맨 아래(BuildDayNightSkySplit의 크기)에서만 짧게 인계돼 딱딱한 경계선 없이
        // 실제 씬으로 섞여든다.
    ...
    // 실제로 두께가 있는 키캡: 밝은 면, 그 아래 드러나는 어두운 옆면, 그 밑의 컨택트 섀도우.
    // Unity 기본 UI 스프라이트는 평면 사각형이라 뒤의 씬 옆에 놓으면 스타일이 없어 보인다.
